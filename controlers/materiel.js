...
const getMateriel = (req, res) => {
  connection.query('SELECT idMateriel, nomMateriel, photoMateriel, description FROM materiel WHERE disponibilite = 1', (error, rows) => {
    if (error) {
...
const updateMateriel = (req, res) => {
  ...
  connection.query(
    'UPDATE materiel SET  nomMateriel = (?), description = (?) WHERE idMateriel = (?)',
    [nomMateriel, description, idMateriel],
    (err, results) => {
      if (err) {
        throw err
      }
